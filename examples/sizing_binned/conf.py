...
def onEvent(evt):
    ...
    if evt["analysis"]["isHit - CCD"]:
        ...
        analysis.sizing.findCenter(evt, "analysis", "binned image - CCD", mask=binnedMask, maxshift=20/binning, threshold=0.5, blur=4)
        ...
        # Fitting model
        t0 = time.time()
        analysis.sizing.sphereModel(evt, "analysis", "offCenterX", "offCenterY", "diameter", "intensity", binnedMask.shape, poisson=True, **modelParams)
        t_full = time.time()-t0
