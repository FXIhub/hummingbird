...
def onEvent(evt):
    ...
    hit, hitscore = analysis.hitfinding.countLitPixels(evt['photonPixelDetectors']['CCD'].data, aduThreshold, hitscoreMinCount)
    ipc.new_data("Hitscore", hitscore)
    hitCounter = numpy.array(analysis.hitfinding.counting(hit))
    print "%d lit pixels (%d accumulated hits)" % (hitscore, hitCounter.sum())
    ...
    correlation = numpy.array(analysis.hitfinding.correlate(evt['pulseEnergies']['pulseEnergy1'].data, hitscore))
    ...
    correlation2D = analysis.hitfinding.correlate2D(evt['pulseEnergies']['pulseEnergy1'].data, hitscore, yMin=correlationMinY, yMax=correlationMaxY, yNbins=correlationNbinsY)
