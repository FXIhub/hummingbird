...
if __name__ == "__main__":
    ...
    parser.add_argument('file', metavar='file', type=str, n=1, 
    ...
    img = numpy.round(img/(3*255))
    img = numpy.array(img, dtype="int16")
    print "Masking out %i/%i pixels (%f.1 %%)." % ((img==0).sum(),img.size,(img==0).sum()/float(img.size)*100))
